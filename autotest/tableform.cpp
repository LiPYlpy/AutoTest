    ...
    ui(new Ui::TableForm)
{
    ...
    //设置表头
    ui->tableWidget->setHorizontalHeaderLabels(QStringList()<<"波道名"<<"波道代号"<<"值"<<"源码HEX"<<"数据保存时间"<<"星上时间"<<"CRC校验"<<"");
...
void TableForm::UpdataUI(QVariantList valueList, QVariantList hexList, QStringList resultList)
{
    for (int i = 0; i<valueList.size(); i++ ) {   //更新value和hex
        ui->tableWidget->setItem(i,2,new QTableWidgetItem(valueList.at(i).toString()));
        ui->tableWidget->setItem(i,3,new QTableWidgetItem(hexList.at(i).toString()));
        if(resultList.at(i)=="true")   //数据无错，设置背景为白色
        {
            ui->tableWidget->item(i,2)->setBackgroundColor(QColor(255,255,255));
            ui->tableWidget->item(i,3)->setBackgroundColor(QColor(255,255,255));
        }
        else   //数据有错设置背景为红色
        {
            ui->tableWidget->item(i,2)->setBackgroundColor(QColor(255,128,128));
            ui->tableWidget->item(i,3)->setBackgroundColor(QColor(255,128,128));
        }
    }
}
...
void TableForm::InitTable(QStringList nameList, QStringList numList)
{
    ...
    for (int i = 0; i < numList.size(); i++) {
        ui->tableWidget->setItem(i,0,new QTableWidgetItem(nameList.at(i)));
        ui->tableWidget->setItem(i,1,new QTableWidgetItem(numList.at(i)));
    }
}
